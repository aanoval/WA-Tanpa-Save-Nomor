    ...
    >
      ...
      <div className="flex items-center space-x-4">
        <motion.div
          ...
          className="text-2xl text-[#005C4B] cursor-pointer"
          aria-label="Toggle language"
        >
          {language === 'id' ? <FiFlag title="Switch to English" /> : <FiFlag title="Switch to Indonesian" />}
        </motion.div>
        <Link href="https://wa.aibiz.id">
          <motion.button
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.95 }}
            className="px-4 py-2 bg-[#005C4B] text-white rounded-lg font-medium shadow hover:bg-[#004238]"
          >
