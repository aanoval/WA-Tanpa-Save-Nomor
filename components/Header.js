...
export default function Header({ language, toggleLanguage, scrolled }) {
  ...
  // Mendapatkan daftar negara dan kode telepon
  const countries = getCountries().map((countryCode) => {
    const countryName = new Intl.DisplayNames([language === 'id' ? 'id' : 'en'], { type: 'region' }).of(countryCode);
  ...
  // Pastikan Indonesia di urutan teratas
  const indonesia = countries.find(country => country.callingCode === '62');
  ...
  return (
    <motion.header
      ...
      className="min-h-screen h-full w-full bg-gradient-to-br from-[#005C4B] to-[#4CAF50] text-white flex flex-col pt-20 px-4 overflow-hidden relative"
      id="form"
    >
      <canvas ref={canvasRef} className="absolute inset-0 z-0 h-full w-full" />
      <nav
        className={`fixed top-0 left-0 w-full z-50 p-4 flex justify-between items-center ${scrolled ? 'bg-[#4CAF50]/90 backdrop-blur-md shadow-md' : 'bg-transparent'}`}
      >
        <motion.h1
      ...
      <div className="relative z-10 max-w-md mx-auto text-center bg-white/20 backdrop-blur-lg rounded-2xl shadow-2xl p-6 mt-16 flex-grow flex items-center justify-center">
